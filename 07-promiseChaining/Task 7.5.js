// Допоміжні дані та функції з оригінального файлу
const products = {
...
function placeOrder(productId, quantity) {
    return getProduct(productId)
        ...
        .then(productInStock => calculateTotal(productInStock, quantity))
        .catch(error => {
...
// Перевірка:
placeOrder(101, 2)
    .then(result => console.log('Тест 7.5a:', result)); // { product: 'Laptop', quantity: 2, unitPrice: 1000, total: 2000 }

placeOrder(103, 1)
    .then(result => console.log('Тест 7.5b:', result)); // { error: 'Out of stock' }

placeOrder(999, 1)
    .then(result => console.log('Тест 7.5c:', result)); // { error: 'Product not found' }